...
def main(input,output):
    ...
    for i in range(len(color)):
        ...
        write_parquet_with_specific_file_name(spark.sparkContext, doform, output, "/DO_{}.parquet".format(color[i]))
        ...
        write_parquet_with_specific_file_name(spark.sparkContext, puform, output, "/PU_{}.parquet".format(color[i]))
                

if __name__ == '__main__':
    input = sys.argv[1]
    output = sys.argv[2]
    spark = SparkSession.builder.appName('D_U_color').getOrCreate()
