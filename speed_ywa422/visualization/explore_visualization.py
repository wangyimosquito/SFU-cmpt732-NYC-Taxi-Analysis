...
Function
Functions below generate static figures of data. These figures help ETL, feature engineering of GBT model, 
and exploration of the nyc taxi speed.
'''

'''
Plot the speed distribution to confirm the valid speed range for ETL
'''
def speed_distribution_plot(data, output):
	data = data.select(data["speed"]).filter(data["speed"]>0)# there are negative outliars
	data = data.toPandas()
	data.hist(column='speed',bins = 200)
	plt.ylabel('frequency')
	plt.savefig(output+'/speed_distribution')

'''
Plot the duration distribution to confirm the valid duration range for ETL
'''
def duration_distribution_plot(data, output):
	...
	plt.savefig(output+'/duration_distribution')
...
def NYC_region_zone_map(data):
	sf = shapefile.Reader("../data/taxi_zone/taxi_zones.shp")
...
def draw_zone_map(ax, sf, shp_dic, heat={}, text=[], arrows=[]):
	...
	plt.xlim(limits[0], limits[1])
	plt.ylim(limits[2], limits[3])
	plt.savefig('../figure/test_gen_output/map')

'''
To see the dynamic of the city in every weekday of 24 hours
'''
def weekday_24hour(output):
	path = '../../speed_aws_ywa422/aws_gen_query_data/24hour_speed'
	files= os.listdir(path)
	for file in files:
		_, suffix = os.path.splitext(file)
		if(suffix == '.csv'):
			data = pd.read_csv(path+'/'+file)
			data["average"] = data["average"].astype("int")
			...
			plt.savefig(output + '/24hour_heatmap_5_years')

'''
Plot the tip percentage and speed to see if there is a connection between them
'''
def tip_speed(data, output):
	...
	plt.savefig(output + '/tip_speed')

def main(inputs, output):
	data, _ = read_ETL(inputs,output)
	# speed_distribution_plot(data, output)
	# duration_distribution_plot(data, output)
	# NYC_region_zone_map(data)
	weekday_24hour(output)
	tip_speed(data, output)
